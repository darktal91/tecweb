...
if ($username eq "") {      #l'utente non è loggato, lo mando alla form di login
  ...
  if ($modifica == 0) {  #è il primo accesso
	...
	if ($cambiati{"citta"} eq "") {
	  $errori = 1;
	  $strerr .= "Il campo citta non può essere vuoto.<br />";
	}
      }
	...
	if ($cambiati{"email"} eq "") {
	  $errori = 1;
	  $strerr .= "Il campo email non può essere vuoto.<br />";
	}
	elsif ($cambiati{"email"} !~ /^([\w\.\-]+)@([A-Za-z0-9\.\-]*[a-zA-Z0-9])\.([a-zA-Z]{2,4})$/) {
	  $errori = 1;
	  $strerr .= "email inserita non valida.<br />";
	}
      }
	...
	$errori = 1;
	$strerr .= "La vecchia password inserita è errata. <br />";
      }
